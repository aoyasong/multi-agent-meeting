...
```text
src/
  index.ts                     # 插件入口，注册 33 tools + 5 commands
  tools/                       # 各类会议工具实现
  commands/                    # slash 命令实现
...
- `storageDir`: 字符串，导出文件目录（默认 `~/.openclaw/meetings`）
...
- 数据库初始化脚本：`docs/postgres-schema.sql`
- 会议纪要：`<storageDir>/<meetingId>/summary.json`（由 `output_generate_summary` 生成，导出文件）
